      ...
      <div className="flex items-center justify-center p-6">
        <div className="w-full max-w-md card p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button disabled={isSubmitting} className="btn btn-primary w-full mt-2">{isSubmitting ? 'Entrando...' : 'Entrar'}</button>
          </form>
          <p className="text-sm text-gray-600 mt-6">¿No tienes cuenta? <Link to="/register" className="text-blue-700 hover:underline">Crear cuenta</Link></p>
        </div>
      </div>
    </div>
  )}
